import { _decorator, Component, instantiate, Label, Node, Prefab, Rect, sp, sys, Vec2, Vec3 } from 'cc';
...
export default class GameUtils 
{
    ...
    public static createItemFromNode<T>(item: new () => T, node: Node, parentNode: Node = null, insert: boolean = false): T 
    {
        if (!node) return null;
    ...
    public static cloneItemFromNode<T>(item: new () => T, node: Node): T 
    {
        if (!node) return null;
        let newNode = instantiate(node);
    ...
    public static createItemFromPrefab<T>(item: new () => T, prefab: Prefab, parentNode: Node): T 
    {
    ...
    return sys.platform == sys.Platform.IOS //|| sys.platform == sys.IPAD;
    }
